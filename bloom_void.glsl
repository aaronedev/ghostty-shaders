// --- Void Bloom (Enhanced) ---
const float BLOOM_THRESHOLD = 0.30;  // Slightly lower threshold for dark theme
const float BLOOM_KNEE = 0.12;
const float BLOOM_INTENSITY = 0.09;  // Boosted intensity for black background
const float BLOOM_RADIUS = 1.2;      // Slightly wider spread

const vec3 samples[24] = vec3[24](
    vec3(0.1693761725038636, 0.9855514761735895, 1.0), vec3(-1.333070830962943, 0.4721463328627773, 0.7071067811865475),
    vec3(-0.8464394909806497, -1.51113870578065, 0.5773502691896258), vec3(1.554155680728463, -1.2588090085709776, 0.5),
...
float softThreshold(float l) { return smoothstep(BLOOM_THRESHOLD - BLOOM_KNEE, BLOOM_THRESHOLD + BLOOM_KNEE, l); }
